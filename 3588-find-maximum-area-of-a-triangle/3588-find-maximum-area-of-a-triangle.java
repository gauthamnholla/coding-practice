...
class Solution {
    public long maxArea(int[][] coords) {
        ...
        for (Map.Entry<Integer, TreeSet<Integer>> entry : xMap.entrySet()) {
            ...
            if (minX != x) ans = Math.max(ans, 1L * Math.abs(x - minX) * base);
            if (maxX != x) ans = Math.max(ans, 1L * Math.abs(x - maxX) * base);
        }
        ...
        for (Map.Entry<Integer, TreeSet<Integer>> entry : yMap.entrySet()) {
            ...
            if (minY != y) ans = Math.max(ans, 1L * Math.abs(y - minY) * base);
            if (maxY != y) ans = Math.max(ans, 1L * Math.abs(y - maxY) * base);
        }
