...
class Solution {
    ...
    int calcMaxTime(int[] time, int val) {
        ...
        int i = 0;
        while (val > 0) {
            if ((val & 1) == 1) max = Math.max(max, time[i]);
            val >>= 1;
            i++;
        }
        return max;
    ...
    public double minTime(int n, int k, int m, int[] time, double[] mul) {
        ...
        while (!pq.isEmpty()) {
            ...
            if (curr.mask == (1 << n) - 1) return curr.cost; // All people transferred

            if (visited[curr.mask][curr.stage][curr.side]) continue;
            ...
            if (curr.side == 0) {
                ...
                for (int[] group : validGroups) {
                    ...
                    if ((curr.mask & groupMask) != 0) continue; // group already transferred
