class Solution {
	...
	private boolean backtrack(int n, boolean[][] matches, int[][] result, int cur, int[] matchesCount)
	{
		if(cur == result.length) return true;
		...
		for(int i = 0; i < n; i++)
		{
			if(result[cur - 1][0] != teams[i] && result[cur - 1][1] != teams[i])
			{
				...
				for(int j = 0; j < n; j++)
				{
					if(i != j && !matches[teams[i]][teams[j]] && result[cur - 1][0] != teams[j] && result[cur - 1][1] != teams[j])
					{
						team1 = teams[i];
						team2 = teams[j];
						break;
